...
const CreateLeagueModal: React.FC<CreateLeagueModalProps> = ({ onClose, onSubmit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.daysOfWeek.length) return alert('กรุณาเลือกวันแข่งอย่างน้อย 1 วัน');

    // Date Validation
    if (formData.startDate < today) return alert('วันที่เริ่มแข่งต้องไม่เป็นวันในอดีต');
    if (formData.endDate && formData.endDate <= formData.startDate) return alert('วันที่สิ้นสุดต้องหลังจากวันที่เริ่มเริ่มแข่ง');
    if (regEndDate && regEndDate < today) {
      return alert(`วันปิดรับสมัคร (${regEndDate}) ต้องไม่เป็นวันในอดีต (กรุณาลดจำนวนวัน หรือเลื่อนวันเริ่มแข่งไปข้างหน้า)`);
    }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-fade-in text-slate-800">
      <div className="bg-white w-full max-w-3xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-zoom-in border border-slate-100">
        ...
        <div className="bg-blue-950 p-8 text-white shrink-0 flex justify-between items-center relative overflow-hidden">
          ...
          <button onClick={onClose} className="text-white/40 hover:text-white transition-all text-2xl relative z-10">&times;</button>
        </div>
