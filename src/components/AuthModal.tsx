...
{/* LOGIN FORM */ }
{
  ...
  type = "email"
  placeholder = "อีเมล"
  className = {`w-full px-4 py-2.5 bg-slate-50 border rounded-xl ${errorsLogin.email ? "border-red-500" : "border-slate-200"
    }`
}
              />
{
...
className = {`w-full px-4 py-2.5 bg-slate-50 border rounded-xl ${errorsLogin.password ? "border-red-500" : "border-slate-200"
  }`}
              />
{
...
className = {`p-3 rounded-xl border-2 transition-all ${regRole === r
...
className = {`w-full px-4 py-2.5 bg-slate-50 border rounded-xl ${errorsSignup.name ? "border-red-500" : "border-slate-200"
  }`}
              />
{
  errorsSignup.name && (
...
type = "email"
placeholder = "อีเมล"
className = {`w-full px-4 py-2.5 bg-slate-50 border rounded-xl ${errorsSignup.email ? "border-red-500" : "border-slate-200"
  }`}
              />
{
  errorsSignup.email && (
...
type = "password"
placeholder = "รหัสผ่าน"
className = {`w-full px-4 py-2.5 bg-slate-50 border rounded-xl ${errorsSignup.password
    ? "border-red-500"
    : "border-slate-200"
  }`}
                />
  ...
  regPassword && (
    <div className="mt-2" >
      <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden" >
        <div
                        className={
    `h-full transition-all duration-300 ${getStrengthColor(
    ...
    < div className = "flex justify-between items-start mt-1" >
      <span
                        className={
    `text-[10px] font-bold ${(passwordResult?.score || 0) < 2
      ? "text-red-500"
...
className = {`w-full px-4 py-2.5 bg-slate-50 border rounded-xl ${errorsSignup.confirmPassword
    ? "border-red-500"
    : "border-slate-200"
  }`}
              />
